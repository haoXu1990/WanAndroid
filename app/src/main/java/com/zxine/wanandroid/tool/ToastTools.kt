package com.zxine.wanandroid.tool
...
fun showToast(text: String?) {
    showToast(context = WanAndroidApp.CONTEXT, text = text)
}

fun showToast(context: Context = WanAndroidApp.CONTEXT, text: String?) {
 ...
 */
fun showToast(context: Context = WanAndroidApp.CONTEXT, resId: Int) {
    if (Thread.currentThread() === Looper.getMainLooper().thread) {
 ...
 * @param text String，显示的内容
 */
fun showLongToast(context: Context? = WanAndroidApp.CONTEXT, text: String?) {
 ...
 * @param resId int，显示内容的字符串索引
 */
fun showLongToast(context: Context? = WanAndroidApp.CONTEXT, resId: Int) {
...
private fun showToast(context: Context? = WanAndroidApp.CONTEXT, text: String?, duration: Int) {
    if (TextUtils.isEmpty(text)) return
    cancelToast()
    if (mToast == null) {
...
fun showToast(context: Context? = WanAndroidApp.CONTEXT, res: Int, duration: Int) {
    cancelToast()
    if (mToast == null) {
